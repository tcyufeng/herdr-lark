...
**一个会话一个群**（不是一个目录一个群）——同一个仓库里你常常开好几个会话做不同的事，
按目录绑会让它们共用一个群、共用一个提问槽位，而你的回复会被投给"最后跑过命令的那个窗格"，
也就是**错的 agent**。也不按窗格绑：关掉 space、移动窗口、resume 之后窗格号就变了，会话 id 不变。

群名是 `🤖 项目名 · 任务标题`，用的就是 Claude 自己生成的那行终端标题，`/clear` 换了任务它会自动改名，
但不会覆盖你手动起的名字。
...
每张同步卡底部都写着**现在轮到谁**：⏳ 还在跑，回合结束后**原地改写**成 ✅ 轮到你，卡在只有你能点的确认框上时是 ⚠️。
只有最新那张带状态，上面的一律降级成指路牌——所以永远只看最后一张。
...
同步要求 agent 在**结束发言之前**把答复复制一份塞进命令里，而结束发言就是回合的终点，后面没有任何钩子可挂。
深在任务里的会话写完答案就停了，手机那头一片安静。

两层兜底，都不碰你终端里正在进行的对话：

- **所有 agent CLI**：一轮结束时一个字都没同步过，就往群里推一张 🔇 卡，并把**终端原文**抄进去。
  能看，但那是终端的截图——表格会折行，长答复会被截断。
- **Claude Code**：`Stop` 钩子从 transcript 里取出这一轮的**真正原文**同步过去，markdown 原样保留。
  agent 自己已经同步掉大半的，它自动闭嘴。可选，往 `settings.json` 里加一条即可，
  见 [examples/hooks](./examples/hooks/README.md)。skill 本身不依赖它。
