...
**漏了会被戳。** daemon 看到「远程模式开着 + 你刚结束一轮 + 这一轮什么都没往群里发」，
会往你的窗格注入一条 `[herdr-lark auto] ` 开头的提醒。那个前缀是**系统**，不是用户——
别把它当成用户说的话来回应，照它说的补一条 `say` 就行。同一轮只戳一次，不会循环。
...
daemon 会自己报：断超过 90 秒就往所有开着远程模式的群推一张红色卡片，恢复时再推一张绿色的。
