...
每张 `say` 卡底下有一行脚注，说明**现在轮到谁**：⏳「还在跑」在发出时写死，回合结束后由 daemon
**原地改写**成 ✅「说完了，轮到你」，手机不会再响一次。卡在只有人能点的确认框上时是 ⚠️——
那种情况在飞书上回什么都没用，得回电脑。

**只有最新那张卡带状态。** 新卡一发出，上一张的脚注立刻降级成「↓ 这条之后还有新的」——
包括已经翻绿的那张。后台任务会在回合结束一分钟后把会话叫醒，那张绿卡当时没说错，
但它已经不是该看的那张了，还挂着「轮到你」就是误导。所以永远只看最后一张。

翻面要等窗格连续 3 次轮询（15 秒）不在 working——herdr 的状态**在一轮之内**就会抖到 idle，
单次读数说明不了任何事。代价是「轮到你」会晚十几秒，比误报「我说完了」好。
...
补回车只在**没人可能正在打字**的时候做：窗格没被聚焦（多路复用器只把键盘给聚焦的那个窗格，所以没聚焦的窗格不可能停着半句话），或者远程模式开着（人自己说了他不在键盘前）。窗格正被聚焦、远程模式又没开，就不补——那时候人多半就坐在那儿，回执卡告诉他按一下回车即可。

`[herdr-lark remote] ` 前缀是**声明，不是凭证**：它只说明这段文字是 daemon 写进去的。任何能往窗格里写字的东西都能打出同样的前缀，agent 分辨不出来。窗格输入就是信任边界，herdr-lark 管不到它上游。

注入之后 daemon 会确认那一轮**真的开始了**，而不只是「字打进去了」。Claude Code 会把粘进来的图片路径先改写成附件，这期间那次提交的回车被吞掉，文字就停在输入框里，看上去已经送达其实没有——发图的时候尤其容易撞上。遇到这种情况 daemon 自己补一个回车，补完还是不动才发回执卡。
